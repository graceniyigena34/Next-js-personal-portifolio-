      ...
      <section className="min-h-screen pt-24 pb-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-gray-50 to-emerald-50">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="relative">
            <div className="absolute left-1/2 transform -translate-x-1/2 w-0.5 h-full bg-emerald-600 hidden lg:block"></div>
              ...
              <div key={i} className={`flex flex-col lg:flex-row mb-8 sm:mb-12 relative ${i % 2 === 0 ? '' : 'lg:flex-row-reverse'}`}>
                <div className={`lg:w-1/2 ${i % 2 === 0 ? 'lg:pr-8 xl:pr-12' : 'lg:pl-8 xl:pl-12'}`}>
                  <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300">
                    <span className="inline-block px-4 py-2 bg-emerald-600 text-white rounded-full text-sm mb-4">{exp.date}</span>
                    <h3 className="text-xl sm:text-2xl font-semibold mb-2">{exp.title}</h3>
                    <h4 className="text-lg text-emerald-600 mb-4">{exp.company}</h4>
                ...
                <div className="hidden lg:block absolute left-1/2 transform -translate-x-1/2 w-5 h-5 bg-emerald-600 rounded-full border-4 border-white shadow-lg" style={{top: '2rem'}}></div>
              </div>
            ))}
          </div>
